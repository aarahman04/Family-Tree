...
- **Last updated:** 2026-08-18 — **Phase 5 checkpoints CP5.1–CP5.9 all landed, plus CP5.10. The epic is NOT fully complete: CP2.3 is corrected to 🔴.** Landed after a **Codespace-loss recovery** (see below). All five CI jobs verified locally against the pushed branch: eslint (0 errors, 1 pre-existing `react-refresh` warning), prettier, root + web typecheck, root **314/314**, web **261/261**, a11y **6/6**, production build. Byte-identical-when-absent SHA256 re-confirmed across 4 display modes × living-indicator on/off against the pre-CP5.1 baseline.
- **Reviews:** three standalone reviews ran, 11 findings total, **8 fixed**. CP5.4–5.6 poster review (5: 3 fixed in `153f793`, 1 escalated to the user as **D-13**, 1 unreachable and recorded). CP5.7+CP5.8 review (5: 3 fixed in `88b58ff` including a **Medium privacy leak** where the living-person gate covered badges but not the branch-merge glyph; 2 recorded). The "two living rules" finding was then re-examined at the user's direction, found to run toward **disclosure** rather than over-caution, and fixed as **CP5.10** (`206a9dd`).
- **Known-unfinished, in priority order:** (1) **CP2.3 — `Confidence = "possible"` is logically unreachable** and its plan-required fixture was never written; needs its own checkpoint to wire `duplicateSuspects` into `classifyConfidence`. (2) Insight mode defeats the `EditorCanvas` SVG memo (Low). (3) Band alternation collapse if generation numbering ever becomes sparse (Low, currently unreachable). (4) `photoCards` non-determinism on the real sample (pre-existing, user-deferred to a dedicated session).
...
| 2.3      | `analysis/confidence.ts`                              | 🔴     | `2aabefe`                    | **Corrected 2026-08-18 from ✅ — partially done.** `ancestryCompleteness`, `classifyConfidence` and `reasons[]` all shipped and D-12 evidence was captured, but the plan's stated test intent is "confirmed/likely/**possible**/unknown fixtures" and no `possible` fixture exists: `classifyConfidence` has no code path that returns it (verified — every `return` yields confirmed/likely/unknown, and `"possible"` appears in `src/`+`web/src/` only in the type union and two doc comments). The level is **logically unreachable**, not merely absent from the 473-sample; the D-12 tally's `possible: 0` assertion encodes that unreachability rather than exercising the level. See the DEFERRED entry in Known gaps. |
...
| 5.10     | Export badge gate: close the living-status disclosure | ✅     | `206a9dd`                    | **Added beyond the plan's checkpoint list**, at the user's direction, to the same standard as the branch-merge-glyph fix. The gate reused `isPresumedLiving`'s 110-year cap while `poster/layout.ts`'s living dot caps nothing, and the disagreement ran toward **disclosure**: with the living opt-in OFF, a person with no death record and a birth year over 110 years back got a green "Living" disc AND the cousin-marriage overlay on the same card. The gate is now "a recorded death unlocks the overlay" — deliberately stricter than the plan's stated definition, which the displayed "Living (presumed)" stat still uses unchanged. Residual is safe-direction only. Whole-tree invariant test asserts no person can be living by the poster's rule while the gate exports their badge. |
...
- **CP5.7/CP5.8 standalone review, Low+Minor (recorded, NOT fixed — each needs its own scoped change):**
  1. **Two different living/deceased rules can appear on the same card — RESOLVED in CP5.10 (`206a9dd`), direction re-examined 2026-08-18.** Originally recorded here as a cosmetic contradiction. It was not: the divergence only occurs for a person with **no death record and a recorded birth year over `MAX_PLAUSIBLE_AGE` back**, and in that case the gate treated them as deceased and **exported** the sensitive overlay while `poster/layout.ts` drew them a green "Living" dot — a real disclosure for a genuinely living centenarian, in the direction of showing *more*, not less. The export gate is now "a recorded death unlocks the overlay", which closes it and makes the gate agree with the dot by construction. `isPresumedLiving` is unchanged and still backs the displayed "Living (presumed)" stat.
