...
- **Last updated:** 2026-08-17 — **CP5.2+5.3 done.** `d26e3fb`. Both-workspace gates green (root 305/305, web 231/231). Byte-identical-when-absent SHA256-reconfirmed. Standalone review found real cousin marriages never produce a `MarriageConnector` (both spouses are blood-placed → chip path only), so the branch-merge glyph now also draws on the chip-anchor line; also fixed unescaped analytics-color/className SVG-attribute interpolation and added `BRANCH_MERGE_CLASS_NAME` shared constant. `DescentConnector.familyId` intentionally NOT added (out of this checkpoint's scope; nothing consumes it). Known gap carried from Phase 4: `Confidence = "possible"` is still unreachable — see Known gaps below. Next: CP5.4.
...
| 5.4–5.9  | Remaining Phase 5                                     | ⬜     | —                            | Not started.                                                                                                                                                                        |
