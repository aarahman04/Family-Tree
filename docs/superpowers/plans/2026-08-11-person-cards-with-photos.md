...
- **AUD-5 — Low, arc.** Living dot: no legend, can collide on min-width cards, hue-only (colorblind). Files: `renderSvg.ts`. Also: the no-photo placeholder `<title>` is generic ("No photo available") — make it name the person ("No photo of {name}") for screen-reader parity with the real photo's "Photo of {name}". (The placeholder's `name` param was dropped when fixing the web typecheck; re-add it deliberately with a test here.)
- **AUD-6 — ✅ RESOLVED 2026-08-16 (240e8ef).** Med, suspected — GATED behind a repro fixture before any layout-engine edit. `layoutBalanced.finalize` didn't reposition disconnected fragments below the chart as its comment claimed; could overlap/clip off-page. Fix: finalize now relocates unowned fragments as one rigid block below the chart, left-aligned. Repro fixture (`tests/poster-balanced-fragments.test.ts`) `it.fails` flipped to a passing regression guard. Files: `layoutBalanced.ts`.
- **AUD-7 — Low, arc.** `?–1900` (birth-unknown) reads as truncated; the living `1974–` form is intended (spec). Files: `layout.ts`.
- **AUD-8 — ✅ RESOLVED 2026-08-16 (240e8ef).** Med, a11y correctness — was DEFERRED out of the styling/dark-mode phase (2026-08-15 audit, finding M4). `ViewMenu` gives every item `role="menuitemcheckbox"` (`ViewMenu.tsx:80`), including pure actions (Fit tree, Fit width, Fit height, Poster scale, Center selection, Reset view). Only "Show photos" and "Focus mode" are real toggles; the rest announce a checked state they don't have. Fix: split the item model so actions render `role="menuitem"` (no `aria-checked`) and only the two toggles keep `menuitemcheckbox`. Behavioral + a11y-gated, so it does NOT belong in a CSS/layout commit — own commit, own review. Files: `web/src/components/editor/ViewMenu.tsx`.
- **AUD-9 — ✅ RESOLVED 2026-08-16 (240e8ef).** Low, a11y + visual-weight — was DEFERRED (2026-08-15 dark-mode global batch). `border-slate-300` control boundaries were 1.49:1 on white, failing WCAG 1.4.11 (3:1). Decision made: text-input boundaries raised to `border-slate-500` (4.76:1, the lightest passing shade), matching the dark side, across `PersonInspector`, `PersonPicker`, `SearchBox`, `PosterExportPanel`, `CreateFamilyTreeWizard`. Bordered buttons kept `slate-300` (their label is the identifying cue, so 1.4.11 doesn't hold their border to 3:1). Pinned failure in `theme-contrast.test.ts` flipped to a positive assertion.
- **AUD-10 — ✅ RESOLVED 2026-08-16 (240e8ef).** Low, test hardening — was DEFERRED (2026-08-15). Fix: gave both async-parse waits a 5000ms timeout and folded the post-rejection assertions into one `waitFor` so they can't race the async settle under concurrency. `web/tests/integration/upload.test.tsx > "an invalid replacement file leaves the original file and tree completely untouched"` flakes ~1-in-4 under full-suite concurrency (~1050ms), confirmed pre-existing at HEAD independent of the dark-mode work across two separate checkpoints (two different failure modes, both timing). Not urgent, not blocking. Do a dedicated hardening pass on its `waitFor` usage (likely a fixed timeout or an assertion racing an async state settle). Files: `web/tests/integration/upload.test.tsx`.
- **AUD-11 — ✅ RESOLVED 2026-08-16 (240e8ef).** Shade decided: `blue-600`→`blue-700` (5.1:1), hover `blue-800`, at all four filled call sites (Download SVG, Export GEDCOM, PersonInspector Save, ErrorBoundary Reload). Low, a11y contrast — was DEFERRED (2026-08-16, PosterExportPanel dark-mode checkpoint). Saturated filled accent buttons carry white text on `bg-blue-600` at ~3.7:1, under the 4.5:1 bar (WCAG 1.4.3). Pre-existing in the light design and identical in both themes (the fill is theme-independent, so no dark variant was added). Same latent failure the emerald chrome CTAs had before commit `207239e` raised them `emerald-600`→`emerald-700` (5.36:1); the fix here is analogous — `blue-600`→`blue-700` (~5.1:1) at the affected call sites. NOT fixed in the dark-mode pass because it changes light-mode button appearance app-wide (a visual decision, not a contained dark-only swap), same bucket as AUD-9. Affects `Download SVG` (`PosterExportPanel.tsx`) and `Export GEDCOM` (`ExportPanel.tsx`); audit for other `bg-blue-600` filled buttons when fixing. Decide the shade, then swap + re-baseline screenshots.
- **AUD-12 — ✅ RESOLVED 2026-08-16 (240e8ef).** Shade decided: section headers `text-slate-400`/`dark:text-slate-500` → muted `text-slate-600`/`dark:text-slate-400`. Low, a11y contrast — was DEFERRED (2026-08-16, menu/picker checkpoint). `AppearanceMenu` section-header labels ("Display mode", "Photo shape") are `text-slate-400` at 11px uppercase, ~2.5:1 on white — below both the 4.5:1 (small-text) and 3:1 bars. Pre-existing in the light design; the dark-mode pass added the mirror `dark:text-slate-500` on `slate-800` (similarly sub-AA). NOT fixed because the AA-passing shade (`slate-600` / `dark:text-slate-400`, muted) makes these deliberately-subtle section dividers read as normal labels — a visual-weight decision, same bucket as AUD-9/AUD-11. Contained to `AppearanceMenu` (2 labels). Files: `web/src/components/editor/AppearanceMenu.tsx`.
